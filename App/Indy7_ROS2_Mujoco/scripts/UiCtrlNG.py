...
# Add JointState for RViz publishing
...
class ROS2Worker(QThread):
    ...
    def run(self):
        ...
        # Publisher for /joint_states (for RViz)
    ...
    def joint_callback(self, msg):
        """Handle joint state updates and publish to /joint_states for RViz"""
        positions = list(msg.actual_pos)
        self.joint_state_updated.emit(positions)

        # Publish to /joint_states for RViz
        js_msg = JointState()
        js_msg.header.stamp = self.node.get_clock().now().to_msg()
        # Joint names must match those in the URDF
        js_msg.name = [f"joint{i}" for i in range(6)]
        js_msg.position = positions[:6]
