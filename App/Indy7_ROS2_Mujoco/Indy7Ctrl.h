...
#include "MuJoCoVisualization.h"  // Separated visualization manager
...
protected:
	...
	virtual BOOL	InitSimulation			(); // New method for simulation
	virtual BOOL	InitConfig				();
	virtual void	DoAgingTest				();

	// Hardware axes (EtherCAT)
	// CAxisNRMKCore**			m_pEcatAxis;
	CAxis**					m_pEcatAxis;
	CSensorNRMKEndTool**	m_pEcatSensor;
	
	// Simulation axes (MuJoCo)
	CRobotRTMuJoCo*			m_pSimRobot;
	
	// Common interface - points to either hardware or simulation
	CAxis**					m_pCurrentAxes;  // Points to either m_pEcatAxis or simulation axes
	...
	mjModel*				m_pMjModel;
	mjData*					m_pMjData;
	// Note: Visualization is now handled by global CMuJoCoVisualizationManager
#endif
